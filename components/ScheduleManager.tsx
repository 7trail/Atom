       ...
       <div className="p-6 border-b border-dark-border bg-dark-panel">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-sm text-dark-muted">Automated triggers and recurring tasks ({timezone})</p>
                </div>
            </div>
       </div>
                       ...
                       <div key={schedule.id} className={`p-4 rounded-lg border flex items-start justify-between gap-4 transition-all ${
                           ...
                           <div className="flex-1 flex items-start gap-4">
                               ...
                               <div className="flex-1 min-w-0">
                                   ...
                                   <div className="text-xs text-dark-muted font-mono mb-3">
                                       {schedule.type === 'cron' ? getCronDescription(schedule.schedule) : formatDate(schedule.schedule)}
                                   </div>
                                   
                                   <div className="flex flex-wrap items-center gap-4 text-xs">
                                       <span className="text-dark-muted">Created: {new Date(schedule.createdAt).toLocaleDateString()}</span>
                                       {schedule.lastRun && (
                                       ...
                                       <div className="flex items-center gap-2 bg-black/20 px-2 py-1 rounded border border-white/5">
                                           <Bot className="w-3 h-3 text-dark-muted" />
                                           <span className="text-dark-muted">Run by:</span>
                                           <select 
                                                value={schedule.agentId} 
                                                onChange={(e) => onUpdateAgent(schedule.id, e.target.value)}
                                                className="bg-transparent border-none text-dark-muted focus:outline-none cursor-pointer hover:text-dark-text"
                                           >
                                                {agents.map(a => (
                           ...
                           <div className="flex items-center gap-2">
                               <button 
                                   ...
                                   className={`p-2 rounded hover:bg-white/5 transition-colors ${schedule.active ? 'text-green-400' : 'text-dark-muted'}`}
